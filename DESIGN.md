# PM Copilot —— 从一句话需求到可运行原型（求职 Demo）
...
> ⚠️ 关键认知（也是面试金句）：加入一个会调 coding agent 的节点，**并不会让整个系统变成 agent**。判断「是 workflow 还是 agent」看的是**谁控制控制流**。我的控制流仍是写死的，只是某一步把活外包给了一个 agent。所以这是 **hybrid**，而不是「一个 agent」。
>
> 「外层 workflow 我控制；原型那一步该放手时放手给 agent。我清楚边界在哪。」——这句话的段位比「我做了个 agent」高一档。
...
**为什么选 LangGraph（对 BEA 的回应）——面试必被追问，提前备好话术：**
...
> 这套话术把「违背 BEA 建议」翻转成「我读懂了 BEA、并有具体理由地做了取舍」——比无脑用框架强得多。
...
2. **它本身就是你架构的可视化**——面试官「看着」你的 BEA 模式跑，你不用干讲。把 transparency 原则做成了卖点。
3. **和你的前端品味一致**（你那个 Astro flipbook 博客说明你在意视觉）。

代价：比 Streamlit 多花 1~2 天搭 React。但这是求职 demo，门面值这个投入。
...
- **示例锚点**：用**你自己做过的产品**当输入（如 math_app / sassy_timer），从「一句话需求」一路跑到「可运行原型」→ 面试可信度拉满。
...
1. **产品判断力**（对 AI 产品岗）：完整产品闭环（需求→评审→指标→复盘），尤其④对抗评审体现「懂 PRD 哪里会出问题」。
2. **Workflow 编排**（对 AI 应用工程师岗）：点名命中 `Prompt Chaining / Parallelization / Evaluator-Optimizer / Augmented LLM / Evaluation`。
3. **Agent 编排 / hybrid 判断力**（资深信号）：⑧把自主 coding agent 当工具编排，并能讲清「为什么外层是 workflow、唯独这一步该放手给 agent」——直接对标 Anthropic 工程范式。

**相对 niche-finder 的优势**：零外部依赖、纯 LLM + 本机 CLI 编排、完全可控、**一定能按时做完**。求职 demo 最怕做不完，这个不会。
...
**与 BEA 的对照（面试主线）：** Prompt Chaining（主链）、Parallelization-Sectioning（④三 critic、⑤b三评委、⑨三验收评委 均并行扇出）、Evaluator-Optimizer（⑤b→prd_draft、⑨→⑧ 两条投票回路 + 收敛即止）、Augmented LLM（②检索）、委托 Agent / hybrid + Orchestrator-Workers（⑧ planner→并行多 coding agent→merge）、Evaluation（⑨ ground-truth 投票）；并能讲清「为何外层是 workflow、唯独 ⑧ 放手给 agent」。

---

## 12. 踩坑记录（真实 debug，面试谈资）
