...
1. **没有真实上下文接地（grounding）——最致命。** 它不知道你公司的代码库、现有产品、团队、排期、历史决策、真实用户。产出的 PRD 读着漂亮却「放之四海皆准」，因此更适合做框架与起草，落到具体业务仍需人接管。`②调研` 只是浅层网搜。
...
4. **原型是一次性 mock，容易让人高估完成度。** ⑧ 出的是单文件假数据 HTML，没后端、没真可行性、没真人用过；⑨ 是宽松判（核心 happy-path 即通过）。看着能跑 ≠ 方案被验证。
...
8. **形式的严谨不等于判断的深度。** 它能产出结构完整、篇幅可观的文档，但其中的判断质量受限于模型本身，**宜作为辅助与起草工具，关键决策仍需人把关**。
